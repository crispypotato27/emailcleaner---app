...
st.markdown("View and manage your unread, spam, and trash emails.") # Updated description
...
if results is not None:
    ...
    # Removed Junk Emails metric:
    # with col3:
    #     st.metric("🧃 Junk Emails", len(results['junk']))
    with col3: # Shifted Trash Emails to col3
        st.metric("🗑️ Trash Emails", len(results.get('trash', [])))

    # Changed tab2 name from "Spam / Junk" to "Spam"
    tab1, tab2, tab3 = st.tabs(["📥 Unread", "🛡 Spam", "🗑 Trash"])
    ...
    # SPAM TAB (formerly SPAM/JUNK TAB)
    with tab2:
        st.subheader("Filter and Manage Spam Emails") # Updated subheader
        # Removed the radio button for choosing between Spam and Junk
        folder_choice = "Spam" # Default to Spam as Junk is removed
        selected_emails = results['spam']
        folder_key = "spam"

        if not selected_emails:
            st.info(f"No spam emails found.") # Updated info message
        else:
            st.warning(f"Found {len(selected_emails)} spam emails") # Updated warning message

            with st.expander(f"📋 Preview first 3 spam emails"): # Updated text
                for i, email in enumerate(selected_emails[:3]):
                    st.markdown(f"**{i+1}.** {email['subject']} - From: {email['sender']}")

            if st.checkbox(f"Show all spam email details"): # Updated text
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.button(f"🗑️ Delete All Spam Emails", type="primary", key=f"delete_{folder_key}"): # Updated button text
        ...
        with col2:
            if st.session_state.get("confirm_delete") and st.session_state.get("delete_folder") == folder_key:
                if st.button("✅ Confirm Delete", type="secondary", key=f"confirm_{folder_key}"):
                    with st.spinner(f"Deleting {len(selected_emails)} spam emails..."): # Updated spinner text
                        ...
                        st.success(f"Deleted {deleted_count} spam emails.") # Updated success message
